...
#define TRAIN_IMAGE "../training_NN/mnist_dataset/train-images-idx3-ubyte"
#define TRAIN_LABEL "../training_NN/mnist_dataset/train-labels-idx1-ubyte"
#define TEST_IMAGE "../training_NN/mnist_dataset/t10k-images-idx3-ubyte"
#define TEST_LABEL "../training_NN/mnist_dataset/t10k-labels-idx1-ubyte"
...
void image_char2double(int num_data, unsigned char data_image_char[][SIZE], double data_image[][SIZE])
{
    ...
    for (i=0; i<num_data; i++){
        for (j=0; j<SIZE; j++){
            double pixel  = (double)data_image_char[i][j] / 255.0;
            if(pixel < 0.6){
                data_image[i][j] = 0.0;
            }
            else{
